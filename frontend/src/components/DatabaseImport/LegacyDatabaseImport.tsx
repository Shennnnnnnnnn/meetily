  ...
  return (
    <Dialog open={isOpen} onOpenChange={() => {}}>
      <DialogContent className="sm:max-w-[600px]" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="text-2xl">{t('database.legacyWelcome')}</DialogTitle>
          <DialogDescription className="text-base pt-2">
            {t('database.legacyQuestion')}
          </DialogDescription>
        </DialogHeader>
            ...
            >
              {importState === 'selecting' || importState === 'detecting' ? (
                ...
                <>
                  ...
                  <span>{t('database.browseDatabase')}</span>
                </>
              )}
            </button>
          </div>
            ...
            <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-green-800">{t('database.databaseFound')}</p>
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            >
              {importState === 'importing' ? (
                <>
                  <Loader2 className="h-5 w-5 animate-spin" />
                  <span>{t('database.importingLegacy')}</span>
                </>
              ) : importState === 'success' ? (
                <>
                  <CheckCircle2 className="h-5 w-5" />
                  <span>{t('database.successLegacy')}</span>
                </>
              ) : (
                <>
                  <Database className="h-5 w-5" />
                  <span>{t('database.importDatabase')}</span>
                </>
              )}
            </button>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-gray-500">{t('database.or')}</span>
              </div>
            </div>
