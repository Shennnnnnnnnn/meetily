...
export function ChunkProgressDisplay({
  ...
  const formatDuration = (ms: number) => {
    ...
    if (hours > 0) {
      return `${hours}${t('progress.hour')} ${minutes % 60}${t('progress.minute')} ${seconds % 60}${t('progress.second')}`;
    } else if (minutes > 0) {
      return `${minutes}${t('progress.minute')} ${seconds % 60}${t('progress.second')}`;
    } else {
      return `${seconds}${t('progress.second')}`;
    }
  };

  const formatTimeRemaining = (ms?: number) => {
    if (!ms || ms <= 0) return t('progress.calculating');
    ...
    <div className={`bg-white border border-gray-200 rounded-lg p-4 ${className}`}>
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <span className="text-sm font-medium text-gray-700">
            {t('progress.completedCount', { completed: progress.completed_chunks, total: progress.total_chunks })}
          </span>
          <span className="text-sm font-medium text-gray-700">
      ...
      <div className="grid grid-cols-4 gap-4 mb-4 text-sm">
        <div className="text-center">
          ...
          <div className="text-gray-600">{t('common.completed')}</div>
        </div>
        ...
        <div className="text-center">
          ...
          <div className="text-gray-600">{t('common.pending')}</div>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mb-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm text-blue-800">
              {t('progress.estimated', { time: formatTimeRemaining(progress.estimated_remaining_ms) })}
            </span>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        <h4 className="text-sm font-medium text-gray-700 mb-2">
          {t('progress.recent', { shown: Math.min(progress.chunks.length, 10), total: progress.total_chunks })}
        </h4>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium">
                      {t('progress.chunk', { id: chunk.chunk_id })}
                    </span>
                    {chunk.duration_ms && (
                ...
                {chunk.error_message && (
                  <div className="mt-1 text-red-700 text-xs">
                    {t('progress.error', { error: chunk.error_message })}
                  </div>
                )}
              </div>
            ))}
        ...
        <div className="mt-4 bg-green-50 border border-green-200 rounded-lg p-3">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-sm font-medium text-green-800">
              {t('progress.complete', { count: progress.total_chunks })}
            </span>
          </div>
        </div>
      )}
  ...
  return (
    <div className={`bg-gray-50 border border-gray-200 rounded-lg p-3 ${className}`}>
      ...
      <div className="text-xs text-gray-600">
        {t('progress.chunkCount', { completed: progress.completed_chunks, total: progress.total_chunks })}
        {progress.processing_chunks > 0 && (
          <span className="ml-2 text-blue-600">
            ({t('progress.processingCount', { count: progress.processing_chunks })})
          </span>
        )}
      </div>
    </div>
