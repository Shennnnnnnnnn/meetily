      ...
      <TooltipProvider>
        <div className="flex flex-col items-center space-y-4 mt-4">
          ...
          <Tooltip>
            ...
            <TooltipContent side="right">
              <p>{isRecording ? t('nav.recordingInProgress') : t('nav.startRecording')}</p>
            </TooltipContent>
          </Tooltip>
            ...
            <>
              {item.id === 'meetings' ? (
                ...
                <Calendar className="w-4 h-4 mr-2" />
                    ...
                    <span className={depth === 0 ? "" : "font-medium"}>{item.id === 'meetings' ? t('nav.meetings') : item.title}</span>
            ...
            <div className="flex flex-col w-full">
                ...
                {isMeetingItem && (
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150">
                    <button
                      ...
                      aria-label={t('common.editMeetingTitle')}
                    >
              ...
              {hasTranscriptMatch && (
                <div className="mt-1 ml-8 text-xs text-gray-500 bg-yellow-50 p-1.5 rounded border border-yellow-100 line-clamp-2">
                    <span className="font-medium text-yellow-600">{t('common.search')}:</span> {matchingResult.matchContext}
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className="fixed top-0 left-0 h-screen z-40">
            ...
            {!isCollapsed && (
              <div className="p-3">
                ...
                <div className="relative mb-1">
                  <InputGroup >
                    <InputGroupInput placeholder={t('nav.searchMeetings')} value={searchQuery}
                ...
                {filteredSidebarItems.filter(item => item.type === 'folder').map(item => (
                  <div key={item.id}>
                    ...
                    >
                      ...
                      <span className="text-gray-700">{item.id === 'meetings' ? t('nav.meetings') : item.title}</span>
                      {searchQuery && item.id === 'meetings' && isSearching && (
                        <span className="ml-2 text-xs text-blue-500 animate-pulse">{t('nav.searching')}</span>
                      )}
                    </div>
            ...
            >
              {isRecording ? (
                <>
                  ...
                  <span>{t('nav.recordingInProgress')}</span>
                </>
              ) : (
                <>
                  <Mic className="w-4 h-4 mr-2" />
                  <span>{t('nav.startRecording')}</span>
                </>
              )}
            </button>
      ...
      <ConfirmationModal
        ...
        text={t('common.deleteMeetingConfirm')}
      ...
      <Dialog open={editModalState.isOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="sm:max-w-[425px]">
          <VisuallyHidden>
            <DialogTitle>{t('common.editMeetingTitle')}</DialogTitle>
          </VisuallyHidden>
          <div className="py-4">
            <h3 className="text-lg font-semibold mb-4">{t('common.editMeetingTitle')}</h3>
            <div className="space-y-4">
              <div>
                <label htmlFor="meeting-title" className="block text-sm font-medium text-gray-700 mb-2">
                  {t('common.meetingTitle')}
                </label>
                <input
                  id="meeting-title"
                  type="text"
