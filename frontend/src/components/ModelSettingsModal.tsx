  ...
  return (
    <div>
      ...
      <div className="space-y-4">
        <div>
          <Label>{t('model.summarization')}</Label>
          <div className="flex space-x-2 mt-1">
            ...
            >
              ...
              <SelectContent className="max-h-64 overflow-y-auto">
                <SelectItem value="builtin-ai">{t('model.builtIn')}</SelectItem>
                <SelectItem value="claude">Claude</SelectItem>
                <SelectItem value="custom-openai">{t('model.customServer')}</SelectItem>
            ...
            {modelConfig.provider !== 'builtin-ai' && modelConfig.provider !== 'custom-openai' && (
              <Popover open={modelComboboxOpen} onOpenChange={setModelComboboxOpen} modal={true}>
                <PopoverTrigger asChild>
                  ...
                  >
                    <span className="truncate">
                      {modelConfig.model || t('model.selectModelEllipsis')}
                    </span>
                ...
                <PopoverContent className="w-[250px] p-0" align="start">
                  <Command>
                    <CommandInput placeholder={t('common.searchModels')} />
                    <CommandList className="max-h-[300px]">
                      {(modelConfig.provider === 'openrouter' && isLoadingOpenRouter) ||
                       ...
                       (modelConfig.provider === 'groq' && isLoadingGroq) ? (
                        ...
                        <>
                          <CommandEmpty>{t('model.noModels')}</CommandEmpty>
                          <CommandGroup>
        ...
        {modelConfig.provider === 'custom-openai' && (
          <div className="space-y-4 border-t pt-4">
            <div>
              <Label htmlFor="custom-endpoint">{t('model.endpoint')} *</Label>
              <Input
                id="custom-endpoint"
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {t('model.endpointDescription')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-model">{t('model.modelName')} *</Label>
              <Input
                id="custom-model"
              ...
              />
              <p className="text-xs text-muted-foreground mt-1">
                {t('model.modelIdentifier')}
              </p>
            </div>

            <div>
              <Label htmlFor="custom-api-key">{t('model.apiKeyOptional')}</Label>
              <Input
                id="custom-api-key"
                type="password"
                value={customOpenAIApiKey}
                onChange={(e) => setCustomOpenAIApiKey(e.target.value)}
                placeholder={t('model.leaveEmpty')}
                className="mt-1"
              />
            </div>
            ...
            <div>
              ...
              >
                <Label className="cursor-pointer">{t('model.advanced')}</Label>
              ...
              {isCustomOpenAIAdvancedOpen && (
                <div className="space-y-3 pl-2 border-l-2 border-muted mt-2">
                  <div>
                    <Label htmlFor="custom-max-tokens">{t('model.maxTokens')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-temperature">{t('model.temperature')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="custom-top-p">{t('model.topP')}</Label>
                    <Input
                      id="custom-top-p"
                      type="number"
                      step="0.1"
              ...
              <div className="absolute inset-y-0 right-0 pr-1 flex items-center space-x-1">
                {apiKey?.trim() && (
                  <Button
                    ...
                    title={isApiKeyLocked ? t('model.unlockToEdit') : t('model.lockToPreventEditing')}
                  >
        ...
        {modelConfig.provider === 'ollama' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-sm font-bold">{t('model.availableOllama')}</h4>
            ...
            {isLoadingOllama ? (
                ...
                {ollamaNotInstalled ? (
                  ...
                  <div className="space-y-4">
                    ...
                    >
                      ...
                      {t('model.downloadOllama')}
                    </Button>
                    <div className="text-sm text-muted-foreground text-center">
                      {t('model.restartAfterInstall')}
                    </div>
                  </div>
                ) : (
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          {isDownloading('gemma3:1b') ? (
                            <>
                              <RefreshCw className="mr-2 h-4 w-4 animate-spin" />
                              {t('model.downloadingNamed', { model: 'gemma3:1b' })}
                            </>
                          ) : (
                            <>
                              <Download className="mr-2 h-4 w-4" />
                              {t('model.downloadRecommended', { model: 'gemma3:1b' })}
                            </>
                          )}
                        </Button>
                        ...
                        {isDownloading('gemma3:1b') && getProgress('gemma3:1b') !== undefined && (
                          <div className="bg-white rounded-md border p-3">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm font-medium text-blue-600">{t('model.downloadingNamed', { model: 'gemma3:1b' })}</span>
              ...
              <ScrollArea className="max-h-[calc(100vh-450px)] overflow-y-auto pr-4">
                {filteredModels.length === 0 ? (
                  <Alert>
                    <AlertDescription>
                      {t('model.noMatches', { query: searchQuery })}
                    </AlertDescription>
                  </Alert>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-muted-foreground">{t('model.withSize')} </span>
