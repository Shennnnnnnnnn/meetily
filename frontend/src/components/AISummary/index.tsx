...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const renderErrorState = () => (
    <div className="w-full p-4 bg-red-50 border border-red-200 rounded-lg">
      ...
      <p className="text-red-500 text-xs mt-2">{t('summary.errorConfiguration')}</p>
    </div>
  );
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h3 className="text-blue-700 font-medium">
            {status === 'processing' ? t('summary.processingTranscript') : t('summary.generating')}
          </h3>
  ...
  if (!hasContent && status === 'completed') {
    return (
      <div className="w-full p-4 bg-gray-50 border border-gray-200 rounded-lg text-center">
        <p className="text-gray-600">{t('summary.noContentAvailable')}</p>
        <p className="text-gray-500 text-sm mt-1">{t('summary.tryNewSummary')}</p>
      </div>
    );
  }
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span>{t('common.copy')} {selectedBlocks.length > 1 ? `${selectedBlocks.length} blocks` : 'block'}</span>
          </button>
          <button
          ...
          >
            <span>🗑️</span>
            <span>{t('common.delete')} {selectedBlocks.length > 1 ? `${selectedBlocks.length} blocks` : 'block'}</span>
          </button>
        </div>
      )}
