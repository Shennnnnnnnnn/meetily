  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-medium text-gray-900">{t('device.title')}</h4>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          {inputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('device.noMicrophone')}</p>
          )}

          {/* Audio Level Meters for Input Devices */}
          {showLevels && inputDevices.length > 0 && (
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <p className="text-xs text-gray-600 font-medium">{t('device.microphoneLevels')}</p>
              {inputDevices.map((device) => {
        ...
        <div className="space-y-2">
          ...
          {outputDevices.length === 0 && (
            <p className="text-xs text-gray-500">{t('device.noSystemAudio')}</p>
          )}
      ...
      <div className="text-xs text-gray-500 space-y-1">
        <p>• <strong>{t('device.microphone')}:</strong> {t('device.microphoneDescription')}</p>
        <p>• <strong>{t('device.systemAudio')}:</strong> {t('device.systemAudioDescription')}</p>
        {isMonitoring && (
          <p>• <strong>{t('device.microphoneLevels')}</strong> {t('device.levelDescription')}</p>
        )}
        {!isMonitoring && inputDevices.length > 0 && (
          <p>• <strong>{t('device.tip')}:</strong> {t('device.testTip')}</p>
        )}
      </div>
    </div>
  );
