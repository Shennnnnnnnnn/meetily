  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
        ...
        {recents.map((code) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={isPinned ? t('summary.unpin', { language: languageLabel(code) }) : t('summary.pin', { language: languageLabel(code) })}
                aria-pressed={isPinned}
                title={isPinned ? t('summary.unpin', { language: languageLabel(code) }) : t('summary.pin', { language: languageLabel(code) })}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? t('summary.defaultLanguage', { language: languageLabel(pinned) })
          : t('summary.defaultLanguageEmpty')}
      </p>
    </div>
  );
}
