...
export function RecordingSettings({ onSave }: RecordingSettingsProps) {
  ...
  const handleNotificationToggle = async (enabled: boolean) => {
    try {
      ...
      toast.error(t('recording.preferenceFailed'));
    }
  };
  ...
  const savePreferences = async (prefs: RecordingPreferences) => {
    ...
    try {
      ...
      toast.success(t('recording.devicePreferencesSaved'), {
        description: t('recording.devicePreferencesDescription', { microphone: micDevice, systemAudio: systemDevice })
      });
    } catch (error) {
      console.error('Failed to save recording preferences:', error);
      toast.error(t('recording.devicePreferencesFailed'), {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 border rounded-lg bg-blue-50">
            <div className="text-sm text-blue-800">
              {t('recording.fileFormat', { format: preferences.file_format.toUpperCase() })}
            </div>
            <div className="text-xs text-blue-600 mt-1">
              {t('recording.timestampHint')} ({preferences.file_format})
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between p-4 border rounded-lg">
        <div className="flex-1">
          <div className="font-medium">{t('recording.startNotification')}</div>
          <div className="text-sm text-gray-600">
