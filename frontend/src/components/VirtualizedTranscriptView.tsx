...
const TranscriptSegment = memo(function TranscriptSegment({
    ...
    const { t } = useI18n();
    const displayText = cleanStopWords(text) || (text.trim() === '' ? t('transcription.silence') : text);
                    ...
                    {isRecording ? (
                        <>
                            ...
                            <p className="text-sm text-gray-600">
                                {isPaused ? t('transcription.paused') : t('transcription.listeningForSpeech')}
                            </p>
                            <p className="text-xs mt-1 text-gray-400">
                                {isPaused ? t('transcription.resumeHint') : t('transcription.speakHint')}
                            </p>
                        </>
                    ) : (
                        <>
                            <p className="text-lg font-semibold">{t('transcription.welcome')}</p>
                            <p className="text-xs mt-1">{t('transcription.startHint')}</p>
                        </>
                    )}
                </motion.div>
            ) : useVirtualization ? (
                ...
                <>
                        ...
                        <div ref={loadMoreTriggerRef} className="flex justify-center items-center py-4 mt-2">
                            {isLoadingMore ? (
                                <div className="flex items-center gap-2 text-gray-500">
                                    ...
                                    <span className="text-sm">{t('transcription.loadingMore')}</span>
                                </div>
                            ) : hasMore && totalCount > 0 ? (
                                <span className="text-sm text-gray-400">
                                    {t('transcription.showingSegments', { loaded: loadedCount, total: totalCount })}
                                </span>
                            ) : null}
                        </div>
                    )}
                    ...
                    {!isStopping && isRecording && !isPaused && !isProcessing && segments.length > 0 && (
                        ...
                        >
                            ...
                            <span className="text-sm">{t('transcription.listening')}</span>
                        </motion.div>
                    )}
                </>
            ) : (
