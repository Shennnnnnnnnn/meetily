  ...
  const handleLanguageChange = async (languageCode: string) => {
    ...
    try {
      ...
      toast.success(t('toast.languageSaved'), {
        description: t('transcription.optimizedFor', { language: languageName })
      });
    } catch (error) {
      ...
      <div className="space-y-2">
        ...
        {isParakeet && (
          <div className="p-2 bg-amber-50 border border-amber-200 rounded text-amber-800">
            <p className="font-medium">ℹ️ {t('transcription.parakeetNoticeTitle')}</p>
            <p className="mt-1 text-xs">{t('transcription.parakeetNoticeBody')}</p>
          </div>
        )}
        ...
        <div className="text-xs space-y-2 pt-2">
          <p className="text-gray-600">
            {t('transcription.current', { language: selectedLanguageName })}
          </p>
          {selectedLanguage === 'auto' && (
            <div className="p-2 bg-yellow-50 border border-yellow-200 rounded text-yellow-800">
              <p className="font-medium">⚠️ {t('transcription.autoWarningTitle')}</p>
              <p className="mt-1">{t('transcription.autoWarningBody')}</p>
            </div>
          )}
          {selectedLanguage === 'auto-translate' && (
            <div className="p-2 bg-blue-50 border border-blue-200 rounded text-blue-800">
              <p className="font-medium">🌐 {t('transcription.translationActive')}</p>
              <p className="mt-1">{t('transcription.translationBody')}</p>
            </div>
          )}
          {selectedLanguage !== 'auto' && selectedLanguage !== 'auto-translate' && (
            <p className="text-gray-600">
              {t('transcription.optimizedFor', { language: selectedLanguageName })}
            </p>
          )}
        </div>
      </div>
